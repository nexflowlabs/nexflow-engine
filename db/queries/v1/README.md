...
The workflow runs: **transform** (set amount) → **expression** (amount > 50) → **condition** (amount >= 50) → **script** (outcome success) or merge path → **httpCall** (test app; step config includes optional `retry.maxAttempts` / `retry.backoffSeconds`) → end. Use this to validate engine + test app together. See [docs/API.md](../../docs/API.md) and [docs/Built-in-Steps.md](../../docs/Built-in-Steps.md#retry-all-built-in-steps) for API and retry details.
